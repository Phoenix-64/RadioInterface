...
from .tuner import WavelogTuneServer
...
class CATBridge:
    ...
    def __init__(self) -> None:
        ...
        self.tuner = WavelogTuneServer(settings.WAVELOG_TUNE_PORT, self._on_tune_request)
    ...
    async def _on_frequency_change(self, freq: int) -> None:
        """SDR frequency change — coalesces rapid scroll steps, latest value wins."""
        ...
        # while the worker is busy. The worker sends only the latest value.
    ...
    async def _sdr_freq_worker(self) -> None:
        """
        Send SDR frequency changes to the rig one at a time.

        While the rig round-trip is in progress any further scroll steps just
        overwrite _pending_sdr_freq. When the rig command finishes we send
        ...
        while True:
            ...
            # If more scroll events arrived while the rig was busy, loop immediately.
            if self._pending_sdr_freq is not None:
                self._sdr_freq_event.set()

    # ================= WaveLog tune request =================

    async def _on_tune_request(self, freq: int, mode: str) -> None:
        """Bandmap cluster click from WaveLog — tune rig and SDR."""
        logging.info(f"[TUNE] WaveLog cluster -> freq={freq} mode={mode}")

        self.current_frequency = freq
        self.current_mode = mode
        # Mark "wavelog" so both the rig monitor and SDR echo are suppressed
        # for SYNC_SUPPRESS seconds, preventing an immediate bounce-back.
        self._mark_sync("wavelog")

        await self.rig.set_frequency(freq)
        await self.rig.set_mode(mode)
        await self.sdr.set_frequency(freq)
        await self.sdr.set_mode(mode)
        await self.wave.broadcast_status(freq, mode)
