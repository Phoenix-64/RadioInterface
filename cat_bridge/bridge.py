...
class CATBridge:
    ...
    def __init__(self) -> None:
        ...
        self.wave = WaveLogClient(
            settings.WAVELOG_URL,
            settings.WAVELOG_TOKEN,
            settings.WAVELOG_RADIO_NAME,
            settings.WAVELOG_MIN_INTERVAL,
        )
    ...
    async def _push_wavelog(self) -> None:
        """Push current state to WaveLog: frequency = TX, frequency_rx = RX (split only)."""
        if not self.tx_freq:
            return
        split = self.split_mode != 1 and self.rx_freq is not None
        await self.wave.push_status(
            self.tx_freq,
            self.rx_freq if split else None,
